...
The rules above are the standard the dataset is held to, and as of **2026-08-06** parts of it do not meet them. An audit of the whole repository found the following, and a phased remediation is under way. Publishing the gap is preferable to letting a reader assume it is not there.

| Gap | Extent | Status |
|---|---|---|
| `namingStatuteUrl`, `statuteUrl` and some `sources[].url` values point at commercial statute mirrors rather than the state's own publisher | 0 <!-- c9:mirror-urls=0,0 --> (down from 145 across 29) | closed: every jurisdiction was migrated to its own publisher, each replacement opened and checked against the cited section, except the four listed in the next row |
...
| `restrictedWords` is an empty array where the state's list has not yet been read | 20 of 51 <!-- c9:restricted-empty=20,51 --> | an empty array here means *not yet researched*, not *the state restricts nothing*. Three of the 20 are settled rather than pending: the state publishes no enumerated list, and the record's `notes` says so <!-- c9:restricted-unjustified=17 --> |
| Some cited URLs had gone dead since they were last opened | 0 open, of 17 found; last measured 2026-08-06 <!-- c9:dead-links=0 measured=2026-08-06 --> | closed: most were re-sourced against the publisher's own successor page, and the remainder turned out not to be dead at all — bot protection, a network condition, or a live page that an automated probe had misread. A dead link is never replaced by a guess, and every replacement was opened in a browser before it was written. Unlike the rows above, this one is not recomputed on every update: whether a URL answers is a fact about the network rather than about the data, and a single probe is not a measurement — a URL can answer on one route and not another. It is therefore measured on demand, twice under different network conditions, and the extent column carries the date that was last done |
