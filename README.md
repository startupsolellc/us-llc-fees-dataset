...
| Gap | Extent | Status |
|---|---|---|
| `namingStatuteUrl`, `statuteUrl` and some `sources[].url` values point at commercial statute mirrors rather than the state's own publisher | 5 URLs in 1 jurisdiction (Wyoming), in `name-rules` and `dba-rules` only (down from 145 across 29) <!-- c9:mirror-urls=5,1 --> | being migrated jurisdiction by jurisdiction, each replacement opened and checked against the cited section; the remaining jurisdiction is blocked on its publisher's reachability, not on research |
